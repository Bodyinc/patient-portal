-- Default provider + revised assignment + auto-generated prescriptions for auto-approved orders.

-- 1. Default provider flag (only one provider may be the default).
alter table public.providers add column if not exists is_default boolean not null default false;
create unique index if not exists providers_one_default on public.providers (is_default) where is_default;

-- 2. Rewritten order-creation trigger:
--    Assignment:
--      - auto-approved (no consultation/followup)      -> default provider
--      - needs approval AND a provider covers the state -> unassigned (admin assigns manually)
--      - needs approval AND no provider for the state   -> default provider
--    Auto-approved orders also auto-generate the prescription and move to 'prescribed'.
...
begin
  ...
  -- Is a provider licensed in the patient's state?
  ...
  -- Auto-approved (Workflow A): generate the prescription automatically and advance to prescribed.
...
drop trigger if exists trg_create_medication_order on public.payments;
create trigger trg_create_medication_order
  after insert on public.payments
  for each row execute function public.create_medication_order_on_payment();
