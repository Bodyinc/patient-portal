  ...
  return (
    <section className="rounded-md border border-[#E6DEFF] bg-white p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="flex items-center gap-2 text-2xl font-semibold text-[#2E00AB]">
        ...
        <div className="text-left sm:text-right">
          <p className="text-xs uppercase tracking-wide text-[#2E00AB]/60">Available balance</p>
          <p className="text-3xl font-semibold text-[#2E00AB]">{formatUsd(wallet.balanceCents)}</p>
        </div>
      </div>

      {wallet.transactions.length > 0 ? (
        <div className="mt-4 border-t border-[#EFE9FF] pt-3">
          <ul className="space-y-2 text-sm">
            {transactions.map((txn) => (
              <li key={txn.id} className="flex items-center justify-between gap-3">
                <span className="min-w-0 truncate text-[#2E00AB]/80">
                  {txn.description ??
                    (txn.type === "referral_reward"
                      ...
                      : txn.type === "payment_applied"
                        ...
                        : "Adjustment")}
                  <span className="ml-2 text-xs text-[#2E00AB]/50">
                    {formatDate(txn.created_at)}
                  </span>
                </span>
                <span
                  className={`shrink-0 font-semibold ${
                    txn.amount_cents >= 0 ? "text-emerald-700" : "text-[#2E00AB]"
                  }`}
                >
                  {txn.amount_cents >= 0 ? "+" : "−"}
          ...
          {wallet.transactions.length > 3 ? (
            <button
              ...
              className="mt-2 text-xs font-medium text-[#2E00AB] underline"
            >
        ...
        <p className="mt-3 border-t border-[#EFE9FF] pt-3 text-sm text-[#2E00AB]/60">
